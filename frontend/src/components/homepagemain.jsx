...
import UsernameEntryPopup from "./UsernameEntryPopup";
...
export default function Homepagemain({
    ...
    const [showInput, setEnterUserName] = useState(false);
    ...
    return (
        <div>
            {showInput && (
                <UsernameEntryPopup
                    onClose={() => setEnterUserName(false)}
                    onSuccess={() => {
                        if (onRefresh) onRefresh();
                        setEnterUserName(false);
                    }}
                    platform={typeof showInput === 'string' ? showInput : 'LeetCode'}
                />
            )}
            <div className={`inner-part overflow-y-auto scroll-auto h-full px-8 py-8 ${showInput ? 'blur-md' : 'blur-0'}`}>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <p className="text-[var(--text-secondary)]">{userStats.platformsConnected.length}/4 Platform{userStats.platformsConnected.length > 1 ? 's' : ' '} Connected</p>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">

                                {platformProgress.map((item) => (
                                    <div className="platform-card flex flex-col py-5 px-6 rounded-lg gap-8 relative overflow-hidden h-fit" style={{ backgroundColor: activeTheme === "night" ? item.bgDark : item.bg }}>
                                        <div
                                        ...
                                        <div className="flex flex-row items-center justify-between z-10">
                                            ...
                                            <div className="flex items-center justify-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    {dashboardData?.[item.platform.toLowerCase()] && (
                                                        <span className="text-[var(--text-secondary)] text-[13px] leading-0">@{dashboardData[item.platform.toLowerCase()].username || dashboardData[item.platform.toLowerCase()].handle}</span>
                                                    )}
                                                </div>
                                            </div>
                                            {dashboardData?.[item.platform.toLowerCase()] && (
                                        ...
                                        <div className={` ${dashboardData?.[item.platform.toLowerCase()]?.total_solved || dashboardData?.[item.platform.toLowerCase()]?.solved ? "flex" : "hidden"} flex-row items-center justify-around`}>
                                            <div className="flex flex-col">
                                                <span className='text-2xl font-semibold text-emerald-500 leading-none'>
                                                    {dashboardData?.[item.platform.toLowerCase()] ? (dashboardData[item.platform.toLowerCase()].easy || dashboardData[item.platform.toLowerCase()].easy_solved || dashboardData[item.platform.toLowerCase()].solved || "-") : "-"}
                                                </span>
                                            ...
                                            <div className="flex flex-col">
                                                <span className='text-2xl font-semibold text-amber-600 leading-none'>
                                                    {dashboardData?.[item.platform.toLowerCase()] ? (dashboardData[item.platform.toLowerCase()].medium || dashboardData[item.platform.toLowerCase()].medium_solved || "-") : "-"}
                                                </span>
                                                <span className='text-sm text-[var(--text-secondary)]'>Medium</span>
                                            </div>
                                            <div className="flex flex-col">
                                                <span className='text-2xl font-semibold text-red-700 leading-none'>
                                                    {dashboardData?.[item.platform.toLowerCase()] ? (dashboardData[item.platform.toLowerCase()].hard || dashboardData[item.platform.toLowerCase()].hard_solved || "-") : "-"}
                                                </span>
                                                <span className='text-sm text-[var(--text-secondary)]'>Hard</span>
                                            </div>
                                        </div>
                                        <div className={`${dashboardData?.[item.platform.toLowerCase()]?.total_solved || dashboardData?.[item.platform.toLowerCase()]?.solved ? "block" : "hidden"}`}>
                                            <div className="flex justify-between flex-row mt-1 text-sm">
                                                <p>Overall Progress</p>
                                                <span className='font-semibold'>
                                                    {dashboardData?.[item.platform.toLowerCase()] ? (dashboardData[item.platform.toLowerCase()].total_solved || dashboardData[item.platform.toLowerCase()].solved || "-") : (item.solved || 0)}
                                                </span>
                                            </div>
                                            <div className="loader">
                                                <div className="bg-white h-3 w-full rounded-xl overflow-hidden">
                                                    <div className={`${activeTheme === "day" ? "bg-black" : "bg-[#3039b5]"} h-full`} style={{ width: `${dashboardData?.[item.platform.toLowerCase()] && dashboardData[item.platform.toLowerCase()].total_questions ? Math.min(100, Math.round((dashboardData[item.platform.toLowerCase()].total_solved / dashboardData[item.platform.toLowerCase()].total_questions) * 100)) : 65}%` }}></div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className={` ${dashboardData?.[item.platform.toLowerCase()] ? "flex" : "hidden"} flex-col gap-2.5`}>
                                            <div className="flex justify-between flex-row py-3 px-5 bg-[var(--primary-bg)] rounded-lg">
                                                <p className='text-md'>Current Rating</p>
                                                <div>
                                                    <FontAwesomeIcon icon="arrow-trend-up" className={dashboardData?.[item.platform.toLowerCase()] && dashboardData[item.platform.toLowerCase()].rating ? 'text-green-600' : 'text-gray-400'} />
                                                    <span className='font-semibold ml-1.5'>{dashboardData?.[item.platform.toLowerCase()] && dashboardData[item.platform.toLowerCase()].rating ? dashboardData[item.platform.toLowerCase()].rating : "N/A"}</span>
                                                </div>
                                            </div>
                                            {/* <div className="flex justify-between flex-row py-3 px-5 bg-[var(--primary-bg)] rounded-lg">
                                                    <p className='text-md'>Current Rating</p>
                                        ...
                                        {!dashboardData?.[item.platform.toLowerCase()] && (
                                            <div className="flex items-center justify-center mt-auto">
                                                <button onClick={() => setEnterUserName(item.platform)} className='p-2 px-5  w-full rounded-md bg-[var(--success)] text-white'>
                                                    Connect
                                                </button>
                                            </div>
                                        )}
        ...
        <div style={{ marginTop: "20px" }}>
          ...
          <img src={user.avatar} alt="avatar" width="80" />
                                        ...
                                        {recentActivity.map((activity) => (
                                            <tr key={activity.id} className="hover:bg-blue-500/5 transition-colors group">
                                                <td className="px-6 py-4">
                                                    <a href={activity.link} target="_blank" rel="noopener noreferrer" className="font-bold text-sm hover:text-blue-400 transition-colors uppercase tracking-tight">
                                                        {activity.problem}
                                                    </a>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${activity.difficulty === 'Easy' ? 'bg-green-500/10 text-green-500 border border-green-500/20' :
                    ...
                    <div className="space-y-8">
                        {/* Upcoming Contests */}
                        <div className="bg-[var(--bg-primary)]/30 backdrop-blur-md p-6 rounded-2xl border border-gray-200/5 shadow-md relative overflow-hidden group">
                            <div className="absolute -top-10 -right-10 w-52 h-52 bg-orange-500/10 blur-[60px] rounded-full"></div>
                            ...
                            <div className="space-y-4 relative z-10">
                                {upcomingContests.map((contest) => (
                                    <div key={contest.id} className="p-4 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-light)] hover:border-blue-500/30 transition-all group/item">
                            ...
                            className="w-full mt-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest border bg-[var(--bg-primary)] border-blue-500/20 text-blue-400 hover:bg-blue-500/10 transition-all hover:border-blue-500/50">
                                Global schedule
                            </button>
                        </div>

                        {/* Achievements */}
                        <div className="bg-[var(--bg-primary)]/30 p-6 rounded-2xl border border-gray-200/5 shadow-md relative">
                        <div className="absolute -bottom-10 -left-10 w-52 h-52 bg-orange-500/10 blur-[60px] rounded-full"></div>
                            <div className="flex items-center gap-3 mb-6">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {[...Array(6)].map((_, index) => {
    const item = unlockedAchievements[index];

    return (
        <div 
            key={index}
            className="aspect-square rounded-xl bg-[var(--bg-primary)] border border-[var(--border-light)] flex items-center justify-center group hover:bg-yellow-500/10 hover:border-yellow-500/30 transition-all cursor-pointer"
        >
            {item ? (
                <img 
                    ...
                    className={`w-15 h-15 object-contain  
                        ${activeTheme !== 'day' 
                            ? 'filter brightness-0 saturate-100 invert sepia hue-rotate-[10deg] saturate-[500%]' 
                            : ''}`
                    }
                />
            ) : (
                <FontAwesomeIcon 
                    icon="medal" 
                    className="text-gray-600 group-hover:text-yellow-500 transition-colors text-xl"
                />
            )}
        </div>
    );
                            ...
                            <button onClick={() => setActiveItem("Achievements")}
                            className="w-full mt-6 py-3 rounded-xl cursor-pointer text-xs font-black uppercase tracking-widest border bg-[var(--bg-primary)] border-blue-500/20 text-blue-400 hover:bg-blue-500/10 transition-all hover:border-blue-500/50">
